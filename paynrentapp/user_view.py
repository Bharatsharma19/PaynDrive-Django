...
def ShowVehicleList(request):
    userdata = {'mobileno': '', 'city': request.GET['city'], 'starttime': request.GET['starttime'],
                'endtime': request.GET['endtime'], 'days': request.GET['dh']}
...
def DisplaySelectedVehicle(request):
    ...
    userdata['amount'] = int(
        d[0])*int(selected_vehicle['price'])+(hr*int(d[1]))
